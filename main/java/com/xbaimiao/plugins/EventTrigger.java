...
public class EventTrigger {
    ...
    public void privat(PrivateMessageEvent event) {
        for (Map.Entry<Class<?>, Method> map : yuq.PRIVATE_MESSAGE.entrySet()) {
    ...
    public void recall(MessageRecallEvent event) {
        for (Map.Entry<Class<?>, Method> map : yuq.RECALL_MESSAGE.entrySet()) {
    ...
    public void dedete(FriendDeleteEvent event) {
        for (Map.Entry<Class<?>, Method> map : yuq.FriendDelete.entrySet()) {
    ...
    public void add(FriendAddEvent event) {
        for (Map.Entry<Class<?>, Method> map : yuq.FriendAdd.entrySet()) {
    ...
    public void join(GroupMemberJoinEvent event){
        for (Map.Entry<Class<?>, Method> map : yuq.GROUP_JOIN.entrySet()) {
    ...
    public void join_invite(GroupMemberInviteEvent event){
        for (Map.Entry<Class<?>, Method> map : yuq.GROUP_JOIN_Invite.entrySet()) {
    ...
    public void leave(GroupMemberLeaveEvent event){
        for (Map.Entry<Class<?> ,Method> map : yuq.GROUP_Leave.entrySet()){
    ...
    public void kick(GroupMemberKickEvent event){
        for (Map.Entry<Class<?> ,Method> map : yuq.GROUP_Kick.entrySet()){
    ...
    public void ban(GroupBanMemberEvent event){
        for (Map.Entry<Class<?> ,Method> map : yuq.GROUP_Ban.entrySet()){
    ...
    public void ban(GroupUnBanMemberEvent event){
        for (Map.Entry<Class<?> ,Method> map : yuq.GROUP_UnBan.entrySet()){
    ...
    public void ban(GroupMemberRequestEvent event){
        for (Map.Entry<Class<?> ,Method> map : yuq.GROUP_New.entrySet()){
    ...
    private static void run(Map.Entry<Class<?>, Method> map,Object v){
        try {
            ...
            yuq.getLogger().error("非法访问异常 " + map.getKey().getName() + map.getValue().getName());
        } catch (InvocationTargetException e) {
            yuq.getLogger().error("调用目标异常 " + map.getKey().getName() + map.getValue().getName());
        } catch (InstantiationException e) {
            yuq.getLogger().error("实例化异常 " + map.getKey().getName() + map.getValue().getName());
        }
    }
