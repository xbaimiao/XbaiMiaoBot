...
class PingMServer {
	...
	private fun ping(message: String, fromGroup: Long) {
		...
		if (msg.startsWith("ping")) {
			if (msg == "ping") {
				yuq.groups[fromGroup]?.sendMessage(mf.newMessage() + "请输入服务器IP 例子\"ping 服务器地址\"")
				return
			}
			if (msg.contains("http")) {
				yuq.groups[fromGroup]?.sendMessage(mf.newMessage() + "你输入的是一个网站吧,并不是mc服务器")
				return
			}
	...
	private fun getMotd(ip: String, port: Int): Message {
		val socket = MCServerSocket.getInstance(ip, port) ?: return mf.newMessage() + "这个服务器是关闭状态"
		val status = socket.getStatus(ProtocolVersion.v1_12_2)
		if (socket.isClosed) return mf.newMessage() + "这个服务器是关闭状态"
		if (!status.isMCServer) return mf.newMessage() + "这不是一个MineCraft服务器."
		...
		if (status.icon.hasServerIcon()) {
			...
			return mf.newMessage() + "服务器地址： $serverip\n" +
					"SRV解析: $isSrv\n" +
					"版本: $version\n" +
